...
breed [people person]
people-own [target speed tolerance grp2 solve small safe local slow connected]
...
to setup
  ...
  create-people number-of-people [
    ...
    set target one-of solutions  ;;randomly pick a solution strategy
    face target  ;;decide to pursue the solution strategy
    set small one-of [true false]  ;;size of the group
    set safe one-of [true false]  ;;does the group feel threat?
    set local one-of [true false]  ;;does the group use local wisdom of crowds?
    set slow one-of [true false]  ;;does the group use fast or slow decision making strategies?
    set connected one-of [true false]  ;;is the group communication well connected or limited?
    set speed 1  ;;how fast the group moves to the solution
    set grp2 0  ;;what strategy did the group use
    set tolerance 0  ;; how long the group pursues a particular strategy
  ]
  ask people
  [set label who]  ;;label each person

  ask solutions  ;;easy solutions are green, complex are red
  [ifelse simple
    [set color green]
    [set color red]
  ]
...
to compare
 ask people
      ...
      [ifelse slow
        [ask target
          [ifelse simple
            ...
            set grp 1] ;small group, well-connected, slow decision rules, simple problems
            [set progress progress + 4
            set grp 5] ;small group, well-connected, slow decision rules, complex problems
          ]
        ]
        [ask target
          [ifelse simple
            [set progress progress + 3
            set grp 9] ;small group, well-connected, fast decision rules, simple problems
            [set progress progress + 3
            set grp 13] ;small group, well-connected, fast decision rules, complex problems
          ]
        ]
      ]
      [ifelse slow
        [ask target
          [ifelse simple
            ...
            set grp 17] ;small group, sparse network structure, slow decision rules, simple problems
            [set progress progress + 3
            set grp 21] ;small group, sparse network structure, slow decision rules, complex problems
          ]
        ]
        [ask target
          [ifelse simple
            [set progress progress + 4
            set grp 25] ;small group, sparse network structure, fast decision rules, simple problems
            [set progress progress + 3
            set grp 29] ;small group, sparse network structure, fast decision rules, complex problems
          ]
        ]
      ]
    ]
    [ifelse connected
      [ifelse slow
        [ask target
          [ifelse simple
            [set progress progress + 3
            set grp 33] ;large group, well-connected network structure, slow decision rules, simple problems
            [set progress progress + 2
            set grp 37] ;large group, well-connected network structure, slow decision rules, complex problems
          ]
        ]
        [ask target
          [ifelse simple
            [set progress progress + 3
            set grp 41] ;large group, well-connected network structure, fast decision rules, simple problems
            [set progress progress + 0
            set grp 45] ;large group, well-connected network structure, fast decision rules, complex problems
          ]
        ]
      ]
      [ifelse slow
        [ask target
          [ifelse simple
            [set progress progress + 2
            set grp 49] ;large group, sparse network structure, slow decision rules, simple problems
            [set progress progress + 1
            set grp 53] ;large group, sparse network structure, slow decision rules, complex problems
          ]
        ]
        [ask target
          [ifelse simple
            [set progress progress + 4
            set grp 57] ;large group, sparse network structure, fast decision rules, simple problems
            [set progress progress + 1
            set grp 61] ;large group, sparse network structure, fast decision rules, complex problems
          ]
        ]
      ]
    ]
  ...
  ask people
  [ set speed speed + [progress] of target
    set speed sqrt speed
...
to go
  ask people [
    ...
    if distance target = 0
      [
        set mainlist lput (list label grp2 safe local count people count solutions) mainlist
...
;       stop
    ...
    ifelse distance target < 1
      ...
      fd speed]
    if tolerance > 25  ;; if the solution strategy does not produce a solution in 10 turns choose a new solution
    [set target one-of solutions
      face target
      set tolerance 0
      compare
...
to record_data
  csv:to-file (word behaviorspace-experiment-name "_Run_" behaviorspace-run-number "_PPL" count people "_SOL" count solutions ".csv") mainlist
end
...
@#$#@#$#@
<experiments>
  <experiment name="FEB_4_2019" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="FEB_4_2019b" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>record_data</final>
    <timeLimit steps="250"/>
  ...
  <experiment name="FEB_6_2019" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>record_data</final>
    <timeLimit steps="500"/>
